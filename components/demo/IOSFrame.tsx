...
function IOSStatusBar({ dark = false, time = "9:41" }: { dark?: boolean; time?: string }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 16px 0", boxSizing: "border-box", position: "relative", zIndex: 20, width: "100%" }}>
      <span style={{ fontFamily: "-apple-system, \"SF Pro\", system-ui", fontWeight: 590, fontSize: 15, lineHeight: "22px", color: c, minWidth: 50 }}>{time}</span>
...
export function IOSDevice({ children, width = 360, height = 780, dark = false }: { children: React.ReactNode; width?: number; height?: number; dark?: boolean }) {
  return (
